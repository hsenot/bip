...
/**
 * Creates JSON ( http://www.json.org/ ) for consumption by an ExtJS form from an ADODB record set
 *
 * @param 		object 		$rs 		- record set object
 * @return 		string		- resulting json string
*/
...
function fs2jsonpaged($rs,$res_num)
{
	if (!$rs) {
		trigger_error("Caught Exception: bad recordset passed to rs2json function.", E_USER_ERROR);
		return false;
    ...
    //Total rows
    $output = '{"success":true,"total_rows":'.$res_num.',"rows":['.$output;
